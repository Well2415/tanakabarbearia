...
import { CalendarIcon, CreditCard, QrCode, ShieldCheck, CheckCircle2, Copy } from 'lucide-react';
...
const GuestBooking = () => {
  ...
  const handleConfirmPayment = async () => {
    ...
    const newAppointments: Appointment[] = formData.serviceIds.map((id, index) => {
      ...
      return {
        ...
        time: formData.time,
        status: 'confirmed' as const, // Auto-confirmed because of "payment"
    ...
    setTimeout(() => {
      toast({
        ...
        description: `Sinal de 50% (R$ ${depositValue.toFixed(2).replace('.', ',')}) aprovado. Seu horário está garantido!`
      });
      navigate('/');
    }, 2000);
  };
  ...
  const pixPayload = storage.getPixKey() ? generatePixPayload(storage.getPixKey(), depositValue) : '';
